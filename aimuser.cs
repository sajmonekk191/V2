        ...
        public void InputManager_OnKeyboardEvent(VirtualKeyCode key, KeyState state)
        {
            if (key == VirtualKeyCode.Q && cbQ.Checked == true)
            {
                switch (state)
                {
                    ...
                    case KeyState.Up:
                        ...
                        calls.DoThing("FindEnemy", hodnoty.XerathQ);
            ...
            if (key == VirtualKeyCode.W && cbW.Checked == true)
            {
                switch (state)
                {
                    case KeyState.Down:
                        Point p = Cursor.Position;
                        calls.DoThing("FindEnemy", hodnoty.XerathW);
            ...
            if (key == VirtualKeyCode.E && cbE.Checked == true)
            {
                switch (state)
                {
                    case KeyState.Down:
                        Point p = Cursor.Position;
                        calls.DoThing("FindEnemy", hodnoty.XerathE);
            ...
            if (key == VirtualKeyCode.R && cbR.Checked == true)
            {
                switch (state)
                {
                    case KeyState.Down:
                        Point p = Cursor.Position;
                        calls.DoThing("FindEnemy", hodnoty.XerathR);
